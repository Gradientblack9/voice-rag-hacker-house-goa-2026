...
def _write_embedded_lite(records: list[dict], path: Path = Path("app/index_lite_data.py")) -> None:
    compact=[{key:value for key,value in record.items() if key != "vector"} for record in records]
    payload=json.dumps(compact,ensure_ascii=False,separators=(",",":")).encode("utf-8")
    encoded=base64.b85encode(gzip.compress(payload,compresslevel=9)).decode("ascii")
    pieces=[encoded[index:index+100] for index in range(0,len(encoded),100)]
    path.write_text("# Generated by scripts/ingest.py; do not edit manually.\nDATA = (\n" + "\n".join(f"    {piece!r}" for piece in pieces) + "\n)\n",encoding="utf-8")
...
def main():
    ...
    store.save(); lite_store.save(); _write_embedded_lite(lite_store.records)
